...
import audio from "./pokemon-appear.mp3"
...
const Component = () => {
  ...
  useEffect(() => {
    if (showCongrats) {
      ...
      const congratsTimer = setTimeout(() => {
        ...
        pokemonAudio.play().catch(e => console.log("Pokemon audio failed:", e));
      } catch (error) {
  ...
  const getTileClasses = (item, prefix) => {
    const baseClasses = "transition delay-150 duration-300 ease-in-out hover:-translate-y-1 hover:scale-105 hover:bg-[#5e3f3b] bg-[#e98074] rounded-full h-12 w-12 sm:h-16 sm:w-16 justify-center flex items-center";
    
    if (matchedNumbers.includes(item)) {
      return `${baseClasses} text-[#def2f1] invisible`;
    } else if ((prefix === 't' && firstval === item) || (prefix === 'r' && secondval === item)) {
  ...
  return (
    <>
      ...
      <div style={{ height: "83%" }} className="flex w-full flex-row">
        {/* Left Grid */}
        <main className={`grid grid-cols-2 outline-double p-4 outline-[#8e8d8a] sm:grid-cols-4 lg:grid-cols-5 h-full w-1/2 gap-3 bg-red-50 mb-5 select-none ${gamePaused ? 'opacity-50 pointer-events-none' : ''}`}>
          {array.map((item) => (
            <div
              key={item}
